...
namespace Grunch.Services
{
    public class CodeWordService : ICodeWordService
    {
        ...
        {
            int adjectiveIndex = GetRandom(adjectives.Length);
            int nounIndex = GetRandom(nouns.Length);
            int verbIndex = GetRandom(verbs.Length);

            return string.Format("{0} {1} {2}", adjectives[adjectiveIndex], nouns[nounIndex], verbs[verbIndex]);
        }
    }
}